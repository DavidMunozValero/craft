#!/usr/bin/env python
"""Run the GSA (Gravitational Search Algorithm) timetabling optimization.

Generates a supply (or loads an existing one), builds the revenue-maximizing
timetabling problem, and solves it with the custom GSA. The discrete part
(scheduled services) is obtained with the conflict-avoiding heuristic.

Examples:
    python scripts/run_gsa.py --seed 42 --pop-size 20 --epoch 50
    python scripts/run_gsa.py --supply data/results/supply_custom.yaml --seed 99
"""
